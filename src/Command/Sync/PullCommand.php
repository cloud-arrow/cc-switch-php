...
/**
 * Pull database from WebDAV remote.
 */
class PullCommand extends Command
{
    ...
    protected function configure(): void
    {
        $this
            ->setName('sync:pull')
            ->setDescription('Pull database from WebDAV remote')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $output->writeln(sprintf('<info>Pulling from %s ...</info>', $config['baseUrl']));
        $output->writeln('<comment>A safety backup will be created before importing.</comment>');

        try {
            $service->pull($config);
            $output->writeln('<info>Pull completed successfully.</info>');
            return Command::SUCCESS;
        } catch (\RuntimeException $e) {
            $output->writeln(sprintf('<error>Pull failed: %s</error>', $e->getMessage()));
            return Command::FAILURE;
        }
    }
